...
template<class T, class objectTraits = Tracing::no_traits, class ddtTraits = Tracing::no_traits> class sllListElement;
    ...
    public:
        ...
        //dllListElement(): data_((pointer) Allocator::malloc(sizeof(value_type))) //, next_(NULL), prev_(NULL)
        dllListElement()
        {
    ...
    public:
        ...
        //sllListElement(): data_((pointer) Allocator::malloc(sizeof(value_type))) {
      sllListElement() {
            next_ = element_pointer_unlogged(NULL);
            //next_.setData(NULL);
            //data_ = (pointer) Allocator::malloc(sizeof(value_type));
        ...
        sllListElement(const_reference data) {//, data_(&data) {
            next_ = element_pointer_unlogged(NULL);
...
#ifdef LOGGING
            typename Logger::object_copy S(TYPEID(input_value_type), type_num, instance_id_, sizeof(input_value_type), type_num, other.instance_id_);
#endif
            //refers.push_back(this);

            iterator_unlogged it1(other.unlogged_begin(), instance_id_);
            iterator_unlogged it2(other.unlogged_end(), instance_id_);
            while (it1 != it2) {
